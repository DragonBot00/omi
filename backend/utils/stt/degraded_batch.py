...
class DegradedBatchProcessor:
    ...
    Thread-safe for single-writer (the receive_data loop) and single-reader
    (the flush timer / recovery path).  The flush() method atomically detaches
    the buffer so new audio immediately goes into a fresh buffer.
    """
    ...
    async def flush(
        ...
        *,
        ...
        try:
            ...
            logger.error('Degraded batch transcription failed: %s uid=%s session=%s', e, self._uid, self._session_id)
            return 0
